...
const PadTable: React.FC = () => {
  const [data, setData] = useState<PadRow[]>([]);
  ...
  const fetchPads = () => {
    ...
    fetch(`/api/Pads?${query.toString()}`)
      .then((res) => res.json())
      .then(setData)
      .catch((err) => console.error("Error fetching pads:", err));
  };
  ...
  //   } catch (err) {
  //     console.error(err);
  //     Swal.fire("Error", "Failed to add pad", "error");
  //   }
  // };
    ...
    try {
      ...
      if (!response.ok) {
        ...
        Swal.fire({
          icon: "error",
          title: "Error",
          text: errorData.message || "Failed to submit pad request.",
          confirmButtonText: "OK",
          customClass: {
            confirmButton:
      ...
      Swal.fire({
        ...
        html: `Your request to add a pad has been submitted.<br>Request ID: <b>${result.request_id}</b><br>It will be effective after admin approval.`,
        confirmButtonText: "OK",
        customClass: {
          confirmButton:
            "bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded",
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: "Failed to submit pad request.",
        confirmButtonText: "OK",
        customClass: {
          confirmButton:
            "bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded",
  ...
  // const handleEdit = async (row: PadRow) => {
  //   const { value: newValue } = await Swal.fire({
  //     title: "Edit Pad",
  //     input: "text",
  //     inputValue: row.pad_name,
  //     inputLabel: "Pad",
  ...
  //       if (!trimmed) {
  //         Swal.showValidationMessage("Pad name cannot be empty");
  //         return false;
  //       }
  //       if (trimmed.toLowerCase() === row.pad_name.toLowerCase()) {
  //         // ไม่ต้องเปลี่ยน ถ้าเหมือนเดิม
  //         return false;
  //       }
  //       try {
  //         const res = await axios.get(
  //           `/api/Pads?search=${encodeURIComponent(
  //             trimmed
  //           )}`
  //         );
  //         if (
  ...
  //           res.data.some(
  //             (r: any) => r.pad_name.toLowerCase() === trimmed.toLowerCase()
  //           )
  //         ) {
  //           Swal.showValidationMessage("This pad already exists");
  //           return false;
  //         }
  //       } catch {
  ...
  //   if (newValue && newValue.trim() !== row.pad_name) {
  //     const token = localStorage.getItem("token");
  //     console.log("TOKEN:", token);
  ...
  //     } catch (err) {
  //       console.error(err);
  //       Swal.fire("Error", "Failed to update pad", "error");
  //     }
  //   }
  // };
      ...
      preConfirm: async (v) => {
        ...
        try {
          ...
          if (
            ...
            res.data.some(
              (r: any) =>
                r.pad_name.toLowerCase() === trimmed.toLowerCase() &&
                r.pad_id !== row.pad_id
            )
          ) {
            Swal.showValidationMessage("This pad already exists");
      ...
      try {
        ...
        if (!response.ok) {
          ...
          Swal.fire({
            icon: "error",
            title: "Error",
            text: errorData.message || "Failed to submit pad update request.",
            confirmButtonText: "OK",
            customClass: {
              confirmButton:
                "bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded",
        ...
        Swal.fire({
          icon: "error",
          title: "Error",
          text: "Failed to submit pad update request.",
          confirmButtonText: "OK",
          customClass: {
            confirmButton:
              "bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded",
  ...
  //     title: "Are you sure?",
  //     text: "Do you really want to delete this pad?",
  //     icon: "warning",
  //     showCancelButton: true,
  ...
  const handleDelete = async (id?: number) => {
    ...
    try {
      ...
      const oldDataRes = await fetch(`/api/Pads/${id}`, {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      if (!oldDataRes.ok) {
        Swal.fire({
          icon: "error",
          title: "Error",
          text: "Cannot load pad data for delete request.",
          confirmButtonText: "OK",
          customClass: {
            confirmButton:
              "bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded",
      ...
      if (!response.ok) {
        ...
        Swal.fire({
          icon: "error",
          title: "Error",
          text: errorData.message || "Failed to submit delete request for pad.",
          confirmButtonText: "OK",
          customClass: {
            confirmButton:
              "bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded",
      ...
      Swal.fire({
        icon: "error",
        title: "Error",
        text: "Failed to submit delete request for pad.",
        confirmButtonText: "OK",
        customClass: {
          confirmButton:
            "bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded",
  ...
  const handleExportCSV = () => {
    ...
    link.href = url;
    link.setAttribute("download", "pads.csv");
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  };

  const columns = useMemo<ColumnDef<PadRow, any>[]>(
    () => [
      {
        header: "#",
        cell: ({ row }) => row.index + 1,
      },
      { accessorKey: "pad_name", header: "Pad Name" },
      // {
      //   header: "Actions",
      //   cell: ({ row }) => (
      ...
      //       </button>
      //       <button
      //         onClick={() => handleDelete(row.original.pad_id)}
  ...
  const sortedData = useMemo(() => {
    return [...data].sort((a, b) => a.pad_id - b.pad_id);
  }, [data]);
            ...
            onClick={() => {
              Swal.fire({
                ...
                preConfirm: async (value) => {
                  ...
                  if (!trimmed) {
                    Swal.showValidationMessage("Pad cannot be empty");
                    return false;
                  }
                  try {
                    const res = await axios.get(
