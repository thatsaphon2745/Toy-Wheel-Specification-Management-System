...
const DateTimeFilter: React.FC<DateTimeFilterProps> = ({
  ...
  const validateDateRanges = () => {
    ...
    const reqStart = parseDate(tempFilters.created_at_start);
    const reqEnd = parseDate(tempFilters.created_at_end);
    const appStart = parseDate(tempFilters.updated_at_start);
    const appEnd = parseDate(tempFilters.updated_at_end);

    if (reqStart && reqEnd && reqStart >= reqEnd) {
      errors.push("Created start date must be before end date");
    }

    if (appStart && appEnd && appStart >= appEnd) {
      errors.push("Updated start date must be before end date");
    }
  ...
  const dropdown = (
    ...
    >
      ...
      <div className="px-6 py-6 space-y-8">
        {(["created", "updated"] as const).map((type, index) => (
          <div key={type} className="space-y-4">
            <div className="flex items-center space-x-3">
              <div
                className={`w-3 h-3 rounded-full ${
                  type === "created" ? "bg-blue-500" : "bg-emerald-500"
                }`}
              />
